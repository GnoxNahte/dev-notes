---
publish: true
aliases:
  - MVP
  - SRT
  - TRS
created: 2026-08-01T02:12:07.978Z
modified: 2026-08-04T00:59:32.434Z
published: 2026-08-04T00:59:32.434Z
tags:
  - todo
cssclasses:
  - img-fill-transparent-bg
---

## Overview

![[90 Code/z_Images/OpenGL/LearnOpenGL - Coordinate Systems.png]]
Image from: [Learn OpenGL - Coordinate Systems](https://learnopengl.com/Getting-started/Coordinate-Systems)

## Breakdown

Spaces explanations:

| Space         | Explanation                                                      | Axes                                                       |
| ------------- | ---------------------------------------------------------------- | ---------------------------------------------------------- |
| Local/Model   | Raw vertices positions after exporting the model                 | -                                                          |
| World         | Translating, Rotating and Scaling the model                      | -                                                          |
| View          | Translating and Rotating all objects to align to camera          | -                                                          |
| Clip          | Divide vec4 by homogenous `w` coordinate                         | Center: Center<br>Range: \[-1, 1]                           |
| Screen/Window | Clip space \[-1, 1] range transformed to \[0, Window Width/Height] | Center: Bottom Left<br>Size: (Window Width, Window Height) |

For more detailed explanations of how each transform work:

- [[01 - Model Transform]]
- [[02 - View Transform]]
- [[03 - Orthographic Projection]]
- [[03 - Perspective Projection]]
- [[04 - Clipping]]

### MVP - Model View Projection

$$
M = M_{\text{proj}} \cdot M_{\text{view}} \cdot M_{\text{model}}
$$

### Model - Model to World

Most common Matrix order: **SRT**

- Translate (if the model has a different pivot)
- Scale
- Rotation
- Translate

$$
M_{\text{model}} = M_T \cdot M_R \cdot M_S
$$

> [!NOTE] If the model has a different pivot, then translate to the other pivot before scaling

> [!IMPORTANT] Order in matrix calculation is M=TRS because...
> When getting the resulting vector, it goes through this order:
>
> $$
> \begin{align}
> \vec{v_2}&=(M_T \ (M_R \ (M_S \cdot \vec{v_1}))) \\
> &=(M_T \cdot M_R \cdot M_S) \vec{v_1}
> \end{align}
> $$
>
> [[1. Matrix Basics - Notation, Multiply, Inverse, Determinant#^354ab8]]

### View - World to Camera

Let camera properties be:

- $\vec{r}$ - Right vector
- $\vec{u}$ - Up vector
- $\vec{d}$ - Look direction vector
- $\vec{p}$ - Camera position

$$
M_{\text{view}}=
\begin{bmatrix}
r_x & u_x & d_x & -\vec{p} \cdot \vec{r} \\
r_y & u_y & d_y & -\vec{p} \cdot \vec{u} \\
r_z & u_z & d_z & -\vec{p} \cdot \vec{r} \\
0 & 0 & 0 & 1
\end{bmatrix}
$$

### Projection - Camera to Clip

#### Orthographic

#### Perspective

### Clip to NDC

Let window properties be:

- $w$ = width
- $h$ = height

$$
M_{\text{proj}}=
\begin{bmatrix}
\frac{2}{w} & 0 & 0 \\
0 & \frac{2}{h} & 0 \\
0 & 0 & 1
\end{bmatrix}
$$

Scale by 2 because NDC's size is 2 as the range is: \[-1, 1]

## Inverse matrices

### Camera to World

Let camera properties be:

- $\vec{r}$ - Right vector
- $\vec{u}$ - Up vector
- $\vec{p}$ - Position vector

$$
M_{\text{cam}}=
\begin{bmatrix}
r_x & u_x & p_x \\
r_y & u_y & p_y \\
0 & 0 & 1
\end{bmatrix}
$$
